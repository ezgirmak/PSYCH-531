...
```{r}
Salaries %>%
  filter(yrs.since.phd <25) %>%
  ggplot(aes(x=yrs.since.phd, y=salary, col=discipline)) +geom_point()+facet_grid(~sex)
```
...
```{r}
lm.fit <-glm(salary~sex+yrs.since.phd+discipline, data =Salaries)
tidy(lm.fit)
```
...
```{r}
int.fit<-glm(salary~ yrs.since.phd*sex +discipline, data = Salaries)
tidy(int.fit)
```
...
```{r}
ggplot(diamonds, aes(x=carat, y=price))+geom_point(alpha=0.1)+ geom_smooth(formula = 'y~exp(x)', method='lm') +facet_grid(~cut)
...
```{r}
diam.fit<- glm(price~carat*cut, data=diamonds)
tidy(diam.fit)
```
