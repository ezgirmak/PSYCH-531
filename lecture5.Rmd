...
```
```{r}
ggplot(data=drinks_long,aes(x=year,y=gallons, col = type))+geom_point()
...
```
```{r}
ggplot(data=drinks_long, aes(x=year, y=gallons)) +geom_line(aes(col =type)) +stat_smooth(se=FALSE)
``` 
...
```{r}
ggplot(data=drinks_long,aes(x=year,y=gallons,col=type))+geom_line()+scale_color_manual(values = c("black","red", "yellow")) +ggtitle('deutschland uber alles')
```
...
```
```{r}
ggplot(data= drinks_long, aes(x = year, y= gallons)) + geom_line(aes(col=type)) +facet_grid(~type)
```
...
What about movies with a lead actor whose name begins with "Ch" Caret, ^ means look for Ch in the beginning of the string
```{r}
...
What about movies with a lead actor whose name ends with "son" $ sign means ends with 'on' expression.
```{r}
...
Use lubridate to clean up dates. 
```{r}
